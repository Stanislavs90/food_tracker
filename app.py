...
def index():
    ...
    for result in results:
        """ 
        result = original datebase values
        single_date = moded values
        """
        single_date = {}
...
def view(date):
    ...
    log_cur = db.execute('select food.name, food.protein, food.carbohydrates, food.fat, food.calories from log_date join food_date on \
        food_date.log_date_id = log_date.id join food on food.id = food_date.food_id\
